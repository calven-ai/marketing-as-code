...
- Python 3.10+ (for `aggregate.py` (upstream helper, not shipped here) and `setup_apify_tasks.py` (upstream helper, not shipped here); only stdlib is required — `requests` is used when available but has a `urllib` fallback)
