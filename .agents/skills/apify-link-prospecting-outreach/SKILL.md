...
description: Find link prospects, their contact details and a pitch angle per target. Use when "build a link prospect list".
license: Apache-2.0
metadata:
  kind: workflow
  area: seo
...
If the runner's client-side wait elapses with the Actor still running on Apify, use `scripts/fetch_run_artifacts.js --run-id <id> --output {base}.json` instead of restarting. If the parent run is missing `SUB_ACTOR_RESULTS` (post-2026-05-20 Actor schema), `.agents/skills/apify-link-prospecting-outreach/scripts/fetch_subactors_from_log.js` resolves sub-Actor runIds from the parent log.
