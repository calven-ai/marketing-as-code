...
description: Find and MX-verify work emails for a list of names and domains. Use when "get emails for these contacts".
license: Apache-2.0
metadata:
  kind: workflow
  area: pipeline
...
Full per-Actor input parameters and example payloads are in [references/apify-actor-usage.md](references/apify-actor-usage.md).
...
Output row schema (16 columns, including `Departments`) and per-format rendering details are in [references/output-formats.md](references/output-formats.md).
