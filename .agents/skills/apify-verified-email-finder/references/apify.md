<!-- source: https://raw.githubusercontent.com/apify/awesome-skills/bcb7fb8935d2f36f0ec4143fe3a40efc0c191f0b/skills/apify-verified-email-finder/SKILL.md | license: Apache-2.0 | fetched: 2026-09-14 -->
...
Full per-Actor input parameters and example payloads are in [references/apify-actor-usage.md](apify-actor-usage.md).
...
```bash
node --env-file=.env ${CLAUDE_PLUGIN_ROOT}/references/scripts/run_actor.js \
  --actor "ACTOR_ID" \
  --input 'JSON_INPUT' \
...
Output row schema (16 columns, including `Departments`) and per-format rendering details are in [references/output-formats.md](output-formats.md).
...
See [references/troubleshooting.md](troubleshooting.md).
