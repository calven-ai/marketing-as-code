...
description: Track brand and competitor citations in AI Overviews, ChatGPT, Perplexity and Gemini on a schedule. Use when "are we cited in AI search".
license: Apache-2.0
metadata:
  kind: workflow
  area: aeo
...
3. Aggregate:
   ...
   - **By content type**: infer from URL path patterns (docs → `/docs/`, `/references/`; blog → `/blog/`; news → known news domains)
...
```bash
cp ${CLAUDE_PLUGIN_ROOT}/references/scripts/config.example.json ./config.json
...
```bash
bash ${CLAUDE_PLUGIN_ROOT}/references/scripts/install_cron.sh --cadence daily --hour 9
```
...
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/references/scripts/run_snapshot.py --config ./config.json
```
...
**Telemetry.** Workflow D runs through `references/scripts/run_snapshot.py`, which already sets `User-Agent: apify-awesome-skills/ai-search-visibility-tracker-...` on every Apify API call. When calling these Actors ad-hoc in Workflows A-C, pass the matching flags so usage is attributed: `--user-agent apify-awesome-skills/apify-ai-search-visibility-tracker` and `--json` (use `--format json` for `apify datasets get-items`), and append `2>/dev/null`. Example:
...
- **Word-boundary brand matching** (`\bbrand\b`, case-insensitive). See `references/citation-matching.md`.
- **Registrable-domain citation matching** (`blog.apify.com` counts as `apify.com`). See `references/citation-matching.md`.
...
`Schedule not firing` -- See `references/scheduling.md` troubleshooting section.
