...
description: Find accounts showing intent from job posts, funding events and LinkedIn activity. Use when "who is in market now".
license: Apache-2.0
metadata:
  kind: workflow
  area: pipeline
...
- Python 3.10+ (for `.agents/skills/apify-buying-signal-detection/scripts/aggregate.py` and `.agents/skills/apify-buying-signal-detection/scripts/setup_apify_tasks.py`; only stdlib is required — `requests` is used when available but has a `urllib` fallback)
...
Or a GitHub Actions workflow (`.github/workflows/<name>.yml`):
