<!-- source: https://raw.githubusercontent.com/apify/awesome-skills/bcb7fb8935d2f36f0ec4143fe3a40efc0c191f0b/skills/apify-google-maps-leads/SKILL.md | license: Apache-2.0 | fetched: 2026-09-14 -->
...
- `leadsEnrichmentDepartments` — default `[]` (any). Enum values are listed in [references/actor-inputs.md](actor-inputs.md).
...
Set every field below on **every** run. Full field reference in [references/actor-inputs.md](actor-inputs.md).
...
Applied to the raw `places.json` in order. Full logic in [references/scoring-and-backfill.md](scoring-and-backfill.md).
...
Full column schema in [references/output-format.md](output-format.md). Twenty columns including `Lead Score`, `Instagram Followers`, `Facebook Followers`, `Backfill Source`.
...
For error recovery patterns shared across Apify skills, see [references/gotchas.md](gotchas.md).
