...
| Actor run `TIMED-OUT` (Actor itself ran past its `timeoutSecs`) | See `references/troubleshooting.md`. Lower `organicResult`, cut keywords, or disable some LLM engines. Almost never the cause — usually it's the client-side wait. |
