...
pointers ("read agents/seo.md") — loses auto-discovery and slash-invocation
in every tool. Also rejected: Claude-specific subagents (`.claude/agents/`) —
Claude-only, and skills-as-roles cover the need portably.

### `strategy/` — market truth

Positioning, messaging hierarchy, ICP and personas, competitive notes and
...
- `data/seo/keywords.csv` — the single source of truth for tracked keywords
  ...
  `data/seo/snapshots/2026-08-31-dataforseo-rankings.csv` — a dated pull.
...
- `README.md` — the registry: tool, mechanism, auth, status, env vars.
